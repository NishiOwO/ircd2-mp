...
void	flush_connections(int fd)
{
	...
	if (fd == me.fd)
	    {
		for (i = hmlen(local) - 1; i >= 0; i--)
			if ((cptr = local[i].value) && DBufLength(&cptr->sendQ) > 0)
				(void)send_queued(cptr);
	    }
	else if (fd >= 0 && (cptr = hmget(local, fd)) && DBufLength(&cptr->sendQ) > 0)
		(void)send_queued(cptr);
}
...
*/
	...
	if (hmlen(local) < 50) /* This part optimized for HUB servers... */
	    {
		if (MyConnect(user))
		    {
			va_list	va;
		...
		for (i = 0; i < hmlen(local); i++)
		    {
			if (!(cptr = local[i].value) || IsServer(cptr) ||
			    user == cptr || !user->user)
				continue;
...
void	sendto_match_servs(aChannel *chptr, aClient *from, char *format, ...)
{
	...
	for (i = fdas.highest; i >= 0; i--)
	    {
		if (!(cptr = hmget(local, fdas.fd[i])) || (cptr == from) ||
		    IsMe(cptr))
			continue;
