...
static	int	chhits = 0, chmiss = 0, irc_chsize = 0;
...
int	add_to_channel_hash_table(char *name, aChannel *chptr)
{
	...
	irc_chsize++;
	if (irc_chsize > _CHANNELHASHSIZE)
...
#if defined(DEBUGMODE) || defined(HASHDEBUG)
		/* Print hash entry */
		if (!irc_strcasecmp(parv[2], "show") && (i = atoi(parv[3])))
		{
			show_hash_bucket(sptr, HashTables, shash, i);
			return 1;
		}
