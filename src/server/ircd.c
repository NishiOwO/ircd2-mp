...
static	int	delayed_kills(time_t currenttime)
{
	...
	for (i = dk_lastfd; i >= j; i--)
	{
		...
		if (!(cptr = local[i].value) || !IsPerson(cptr))
		{
...
static	time_t	check_pings(time_t currenttime)
{
	...
	for (i = hmlen(local) - 1; i >= 0; i--)
	    {
		if (!(cptr = local[i].value) || IsListener(cptr))
			continue;
...
{
#ifdef _WIN32
	strcpy(mp->username, "win32-dummy-user");
#else
	struct	passwd	*p;
...
#ifdef _WIN32
	strcpy(mp->user->username, "win32-dummy-user");
#else
