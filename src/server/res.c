	...
	while (hptr->ancount-- > 0 && cp && cp < eob) {
		...
		case T_A :
			...
			if (len < HOSTLEN)
			{
				...
				if (irc_strcasecmp(hp->h_name, hostbuf) &&
		...
		case T_CNAME :
			...
			if (hp->h_name && irc_strcasecmp(hp->h_name, hostbuf))
			{
				*alias = (char *)MyMalloc(len + 1);
				(void)strcpy(*alias++, hostbuf);
				*alias = NULL;
...
static	void	rem_cache(aCache *ocp)
{
	...
	** pointer.
	*/
	for (hashv = hmlen(local) - 1; hashv >= 0; hashv--)
		if ((cptr = local[hashv].value) && (cptr->hostp == hp))
			cptr->hostp = NULL;
	/*
	 * remove cache entry from linked list
	 */
