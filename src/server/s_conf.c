...
int	rehash(aClient *cptr, aClient *sptr, int sig)
{
	...
	for (i = 0; i < hmlen(local); i++)
		if ((acptr = local[i].value) && !IsMe(acptr))
		    {
			/*
...
int	find_conf_flags(char *name, char *key, int stat)
{
	...
	for (tmp = conf; tmp; tmp = tmp->next)
 		if ((tmp->status == stat) && tmp->passwd && tmp->name &&
		    (irc_strncasecmp(key, tmp->passwd, l) == 0) &&
		    (match(tmp->name, name) == 0) &&
...
void do_kline(int tkline, char *who, time_t time, char *user, char *host, char *reason, int status)
{
	...
	{
		if (0==irc_strcasecmp(aconf->host, host) && 
			0==irc_strcasecmp(aconf->name, user))
		{
	...
	{
		if (!(acptr = local[i].value) || !IsPerson(acptr)
			|| IsKlineExempt(acptr))
		{
			continue;
		}
...
int	m_untkline(aClient *cptr, aClient *sptr, int parc, char **parv)
{
	...
	*host++ = '\0';
	...
	{
		if (0==irc_strcasecmp(tmp->host, host) && 
			0==irc_strcasecmp(tmp->name, user))
		{
			if (tmp == tkconf)
				tkconf = tmp->next;
			else
