...
static void	introduce_server(aClient *cptr, aClient *server)
{
	...
	{
		if (!(acptr = hmget(local, fdas.fd[i])) || !IsServer(acptr) ||
...
int	m_server_estab(aClient *cptr, char *sid, char *versionbuf)
{
	...
	for (i = 0; i < hmlen(local); i++)
		if (local[i].value && IsServer(local[i].value))
		    {
			ircstp->is_ref++;
...
static	void	report_myservers(aClient *sptr, char *to)
{
	...
	for (i = fdas.highest; i >= 0; i--)
	{
		if (!(acptr = hmget(local, fdas.fd[i])))
		{
			continue;
		}
...
int	find_server_num(char *sname)
{
	...
	while (i < server_num)
	    {
		if (!irc_strcasecmp(server_name[i], sname))
			break;
		i++;
	    }
	if (i < server_num)
